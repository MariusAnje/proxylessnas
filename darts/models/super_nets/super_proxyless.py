...
class SubProxylessNASNets(ProxylessNASNets):

    def __init__(self, width_stages, n_cell_stages, conv_candidate_space, stride_stages,
                 n_classes=1000, width_mult=1, bn_param=(0.1, 1e-3), dropout_rate=0):
        self._redundant_modules = None
        self._unused_modules = None
        ...
        for width, n_cell, s in zip(width_stages, n_cell_stages, stride_stages):
            conv_candidates = []
            for t in conv_candidate_space[j]:
                conv_candidates.append(candidate_choices[t])
        ...
        classifier = LinearLayer(last_channel, n_classes, dropout_rate=dropout_rate)
        super(SubProxylessNASNets, self).__init__(first_conv, blocks, feature_mix_layer, classifier)
