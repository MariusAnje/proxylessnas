...
class QuantCIFARProxylessNASNets(ProxylessNASNets):
    ...
    def __init__(self, width_stages, n_cell_stages, conv_candidates, stride_stages,
        ...
        first_conv = MixedEdge(candidate_ops=build_candidate_ops(['Identity'], 3, 3, 1, 'weight_bn_act',), )
        ...
        feature_mix_layer = MixedEdge(candidate_ops=build_candidate_ops(['Identity'], width, width, 1, 'weight_bn_act',), )

        classifier = nn.ModuleList([
            LinearLayer(512 * 4 * 4, 1024, dropout_rate=dropout_rate),
            LinearLayer(1024, n_classes, dropout_rate=dropout_rate)
            ])
        super(QuantCIFARProxylessNASNets, self).__init__(first_conv, blocks, feature_mix_layer, classifier)
...
class CIFARProxylessNASNets(ProxylessNASNets):
    ...
    def __init__(self, width_stages, n_cell_stages, conv_candidates, stride_stages,
        ...
        for i, width in enumerate(width_stages, 0):
            ...
            conv_op = MixedEdge(candidate_ops=build_candidate_ops(conv_candidates, input_channel, width, 1, 'weight_bn_act', None), )
        ...
        super(CIFARProxylessNASNets, self).__init__(first_conv, blocks, feature_mix_layer, classifier)
